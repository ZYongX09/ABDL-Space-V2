...
## [2026-06-10 18:02] 2d42ece 审查报告 — beta-register 独立布局

### Bug #1 — P1 🔴 桌面端表单完全无法滚动（功能阻断）
- **文件**：`client/src/App.jsx:172`
- **问题**：独立布局的容器 `<div className="page-transition-enter" style={{ minHeight: '100vh', padding: '20px 16px' }}>` **没有 overflow 设置**。其父级 `.app-layout` 在桌面端（>768px）是 `display: flex; height: 100vh; overflow: hidden;`（`global.css:621-623`），导致内容超出 100vh 时被裁剪且无法滚动。
- **实测证据**（headless Chrome 1280×800，端口 4175 预览构建）：
  - 容器实际高度 `app-layout.scrollHeight = 1513px`，`clientHeight = 800px`
  - `getComputedStyle(.app-layout).overflow = "hidden"`
  - `body.scrollHeight = 800px`，`html.scrollHeight = 800px` —— 整页都不可滚
  - `End` 键 / 鼠标滚轮（2000px）/ `PageDown` 全部无效，`scrollY` 恒为 0
  - **对比** `/register` 页面：`.app-main-content` 有 `overflow-y: auto`，`PageDown` 后 `scrollTop = 415`，正常可滚
- **影响**：viewport 高度小于 ~1200px 的桌面端（多数笔记本），用户看不到密码、确认密码、邀请码、安全验证、协议勾选、提交按钮。**注册流程被卡死**。
- **建议修复**（任选其一）：
  ```jsx
  // 方案 A（推荐，最小改动）：给容器加 overflow-y: auto
  <div className="page-transition-enter" style={{ minHeight: '100vh', padding: '20px 16px', overflowY: 'auto' }}>
  ```
  ```jsx
  // 方案 B：把容器塞进 app-main-content，沿用现有滚动容器
  // 方案 C：给容器加 height: 100vh（而非 minHeight）配合 overflow-y: auto
  ```
- **复现命令**：
  ```bash
  cd client && npx vite build && npx vite preview --port 4175
  google-chrome-unstable --headless --no-sandbox --window-size=1280,800 --screenshot=beta.png http://localhost:4175/beta-register
  # 截图可见：表单在「密码」处被切掉，看不到提交按钮
  ```
- **状态**：待修复

---

### Bug #2 — P3 🟡 重复渲染 ScrollToTop
- **文件**：`client/src/App.jsx:165, 173`
- **问题**：`/beta-register` 分支内层又渲染了一个 `<ScrollToTop />`，加上外层那个（line 165）共两个。两者都会跑 `window.scrollTo(0,0)` 和 `document.title = ...`，重复执行。
- **影响**：title 重复赋值、scrollTo 重复调用。功能无影响，但浪费 work，且容易让人误以为"在 beta 页没生效"而去排查。
- **建议**：删掉分支内的 `<ScrollToTop />`（line 173），外层那个已经覆盖所有路由。
- **状态**：待修复（不阻塞）

---

### Bug #3 — P3 🟡 NotificationProvider 跨分支重建
- **文件**：`client/src/App.jsx:170, 217`
- **问题**：`NotificationProvider` 同时挂在两个分支里。React 路由切换时，从 `/beta-register` 离开会卸载该 Provider 再重新挂载，下方 `ToastPopup` 持有的 toasts 状态会丢失。
- **影响**：当前 BetaRegister 注册成功后停留在本页（不跳转），所以「离开丢 toast」的场景不常触发。但若用户在 beta 页面看到错误 toast 后点浏览器后退/前进或导航到 `/login`，那条 toast 会被清空。ToastPopup 提示会"消失"。
- **建议**：把 `NotificationProvider`（以及 `NsfwProvider`）上移到 `<div className="app-layout">` 之外，与 `<MobileHeaderProvider>` 同级，作为全局 Provider，避免分支差异：
  ```jsx
  ...
  ```
  这样 Provider 状态跨路由保留，且去重。同时省去两处挂载的代码重复。
- **状态**：建议修复（不阻塞）
...
**核心思路正确**——把 `/beta-register` 抽成独立布局确实能让"创始成员计划"页面没有任何其他功能入口，符合产品意图。代码风格、Provider 选择（Notification/Nsfw）、保留 ToastPopup 错误提示、`minHeight: 100vh` 兜底都是合理的。

**但有一个**功能级阻断 Bug**（#1）**：在桌面端 viewport < ~1200px 时，表单下半部分被 `.app-layout { overflow: hidden; height: 100vh }` 裁剪，且整页不能滚，用户**无法完成注册**。这是 P1，建议修完再 merge。

Bug #2 / #3 是 P3 代码洁询，可一并清理。**核心修复建议**：Bug #1 加 `overflowY: 'auto'`；顺手把 #2 删掉内层 ScrollToTop；#3 把 Provider 上提一层。

### 审查通过条件

- [x] 逻辑正确：路由切换、Provider 包裹范围正确
- [x] 类型无问题（纯 JSX）
- [x] 边界条件：mobile (≤768px) 下 `.app-layout` 变 `overflow: visible`，可正常滚（已确认 OK）
- [x] 异步处理：N/A
- [x] React hooks：无依赖数组问题
