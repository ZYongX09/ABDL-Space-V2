...
// Mastodon 兼容路径 /@:username —— 通过后端 /api/users/:username 精确匹配，
// 命中后用 SPA 内跳转（replace）到 /user/:id，避免整页刷新。
// 注意：不能用 users/search（LIKE 模糊匹配，limit=1 会跳到错误用户）。
function MastodonProfile() {
  ...
  useEffect(() => {
    ...
    usersAPI.getUser(username)
      .then(({ user }) => {
        if (cancelled) return;
        if (user && user.id) {
          // 额外校验：避免后端把纯数字 username 当成 ID 返回了别的用户
      ...
      .catch(() => {
        if (!cancelled) setError(`用户 "@${username}" 不存在`);
      });
  ...
  return (
    <div style={{ textAlign: 'center', padding: '4rem' }}>
      <i className="fa-solid fa-spinner fa-spin text-2xl" style={{ color: 'var(--primary)' }} />
      <p style={{ marginTop: 16 }}>正在跳转 @{username} 的个人主页...</p>
    </div>
  );
}
