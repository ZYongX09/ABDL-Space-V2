...
function MastodonProfile() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/api/users/search?q=${encodeURIComponent(username)}&limit=1`, { credentials: 'include' })
      .then(r => r.json())
      .then(data => {
      ...
      .catch(() => { setError('查找用户失败'); setLoading(false); });
  }, [username]);
  ...
  if (loading) {
    return (<div style={{ textAlign: 'center', padding: '4rem' }}><i className="fa-solid fa-spinner fa-spin text-2xl" style={{ color: 'var(--primary)' }} /><p style={{ marginTop: 16 }}>正在跳转 @{username} 的个人主页...</p></div>);
  }
  if (error) {
    return (<div style={{ textAlign: 'center', padding: '4rem 1rem' }}><i className="fa-solid fa-user-slash" style={{ fontSize: 48, color: 'var(--text-muted)', marginBottom: 16, display: 'block' }} /><h2>{error}</h2><a href="/" style={{ color: 'var(--primary)', marginTop: 16, display: 'inline-block' }}>返回首页</a></div>);
  }
  return (
    <div style={{ textAlign: 'center', padding: '4rem 1rem', maxWidth: 480, margin: '0 auto' }}>
      <div className="card" style={{ padding: '2rem' }}>
        <p style={{ marginBottom: '1rem' }}>如果页面没有自动跳转：</p>
        <a href={profileUrl} className="btn btn-primary" style={{ minWidth: 200, display: 'inline-block', marginBottom: 8 }}><i className="fa-solid fa-arrow-up-right-from-square mr-2" />点击访问个人主页</a><br/>
        <button className="btn btn-outline" onClick={handleCopy} style={{ minWidth: 200 }}><i className={`fa-solid ${copied ? 'fa-check' : 'fa-copy'} mr-2`} />{copied ? '已复制' : '复制链接'}</button>
      </div>
    </div>
  );
}
